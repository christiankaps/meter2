...
| Reading entry input state | Implemented | New reading forms start with the date field, infer date-only versus date-time from the typed value, keep the value field empty, support compact date input such as `01062026`, allow `+` and `-` day stepping, and hide non-blocking warnings while the value is actively being typed. |
